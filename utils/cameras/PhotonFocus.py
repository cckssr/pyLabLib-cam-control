...
class IMAQPhotonFocusCameraThread(BaseIMAQPhotonFocusCameraThread):
    ...
    def add_trigger_monitor(self, name, source, line, polarity="low"):
        ...
        if self.device:
            ...
            self.v["trigger_monitors", name] = (
                self.device.read_trigger(source, line, polarity),
                0,
            )  # state, counter
    ...
    def _get_trigger_mode(self):
        if self.device.dv["triggers_in_cfg"]:
            return "in_ext"
        elif dict(self.device.dv["triggers_out_cfg"]).get(self.out_trigger_dst) == (
            "high",
            "vsync",
        ):
            return "out"
    ...
    def _apply_additional_parameters(self, parameters):
        ...
        if (new_trig_mode != curr_trig_mode) and (
            (new_trig_mode == "in_ext") or (curr_trig_mode == "in_ext")
        ):
            self.clear_acquisition()
            self.device.clear_all_triggers()
            if new_trig_mode == "in_ext":
                self.device.configure_trigger_in(
                    self.in_trigger_src[0], self.in_trigger_src[1], "high", "buffer"
                )
        if new_trig_mode == "out":
            self.device.configure_trigger_out(
                self.out_trigger_dst[0], self.out_trigger_dst[1], "high", "vsync"
            )
        else:
            self.device.configure_trigger_out(
                self.out_trigger_dst[0], self.out_trigger_dst[1], "high", "disable"
            )
    ...
    def _add_attribute(self, name, attribute, value):
        ...
        if attribute.kind in {"INT", "UINT"}:
            self._record_attribute(
                ...
                attribute,
                indicator=indicator,
                rng=(attribute.min, attribute.max),
            )
            self.add_integer_parameter(
                name,
                attribute.name,
                limits=(attribute.min, attribute.max),
                indicator=indicator,
            )
...
class BlackOffsetParameter(cam_gui_parameters.IGUIParameter):
    def add(self, base):
        self.base = base
        self.base.add_check_box(
            "change_bl_offset", "Change black level offset", False, add_indicator=False
        )
        self.base.add_num_edit(
            "bl_offset",
            value=3072,
            limiter=(0, None, "coerce", "int"),
            formatter="int",
            label="Black level offset",
        )
        self.base.vs["change_bl_offset"].connect(
            controller.exsafe(lambda v: self.base.set_enabled("bl_offset", v))
        )
...
class ROIGUIParameter(cam_gui_parameters.ROIGUIParameter):
    def _is_diff_update_allowed(self, oldv, newv):
        return oldv and all(
            [
                (cr.max - cr.min == pr.max - pr.min) and (cr.bin == pr.bin)
                for (cr, pr) in zip(oldv, newv)
            ]
        )
...
class PhotonFocusCameraSettings_GUI(GenericCameraSettings_GUI):
    _roi_kind = "minsize"
    _param_dependencies = {
        "cfr": ["trigger_interleave", "frame_period"],
        "trigger_interleave": ["cfr", "frame_period"],
    }

    def get_basic_parameters(self, name):
        """Get basic GUI parameters, which can be shared between different cameras"""
        if name == "exposure":
            return cam_gui_parameters.FloatGUIParameter(
                self,
                "exposure",
                "Exposure (ms)",
                limit=(0, None),
        ...
        if name == "bl_offset":
            return BlackOffsetParameter(self)
        if name == "status_line":
            return cam_gui_parameters.BoolGUIParameter(
                self, "status_line", "Status line", default=True
            )
        if name == "perform_status_check":
            return cam_gui_parameters.BoolGUIParameter(
                self,
...
class PhotonFocusIMAQCameraSettings_GUI(PhotonFocusCameraSettings_GUI):
    ...
    def setup_settings_tables(self):
        super().setup_settings_tables()
        self.add_parameter(
            cam_gui_parameters.BoolGUIParameter(
                self, "output_vsync", "Output VSync trigger", indirect=True
            ),
            "advanced",
            row=1,
        )
...
class PhotonFocusCameraDescriptor(ICameraDescriptor):
    ...
    def iterate_cameras(cls, verbose=False):
        if verbose:
            print("Searching for PhotonFocus cameras (might take several minutes)")
        try:
            ...
            return
        pf_cams = [
            (p, d) for (p, d) in pf_cams if d.manufacturer != "RS-232"
        ]  # these usually don't have cameras, but can lead to very long polling times
        if len(pf_cams) == 0:
            if verbose:
                print("Found no PhotonFocus cameras\n")
            return
        if verbose:
            print(
    ...
    def _detect_interfaces(cls, verbose=False):
        if cls._imaq_interfaces is None:
            try:
                ...
                if verbose:
                    print(
                        "Error loading or running the IMAQ library: required software (NI IMAQ) must be missing\n"
                    )
...
class PhotonFocusSiSoCameraDescriptor(PhotonFocusCameraDescriptor):
    ...
    def _detect_interfaces(cls, verbose=False):
        if cls._siso_boards is None:
            try:
                ...
                if verbose:
                    print(
                        "Error loading or running the Silicon Software library: required software (Silicon Software Runtime Environment) must be missing\n"
                    )
    ...
    def generate_description(cls, idx, cam=None, info=None):
        ...
        for i in range(len(siso_boards)):
            ...
            for n, p in [
                ("DualAreaGray16", [0, 1]),
                ("Acq_SingleBaseAreaGray", [0]),
                ("Acq_DualBaseAreaGray", [0, 1]),
                ("SingleAreaGray16", [0]),
            ]:
    ...
    def make_thread(self, name):
        return SiliconSoftwarePhotonFocusCameraThread(
            name=name, kwargs=self.settings["params"].as_dict()
        )
...
class PhotonFocusBitFlowCameraDescriptor(PhotonFocusCameraDescriptor):
    ...
    def _detect_interfaces(cls, verbose=False):
        if cls._bitflow_interfaces is None:
            try:
                ...
                if verbose:
                    print(
                        "Error loading or running the BitFlow library: required software (BitFlow SDK and BFModule library) must be missing\n"
                    )
    ...
    def generate_description(cls, idx, cam=None, info=None):
        ...
        for i, fginfo in enumerate(bitflow_interfaces):
            ...
            try:
                cam = PhotonFocus.PhotonFocusBitFlowCamera(
                    bitflow_idx=fginfo.idx,
                    bitflow_camfile=camfile,
                    pfcam_port=pfcam_port,
                )
