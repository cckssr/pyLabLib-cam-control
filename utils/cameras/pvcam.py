...
class TriggerModeParameter(cam_gui_parameters.IGUIParameter):
    ...
    def setup(self, parameters, full_info):
        ...
        if "parameter_ranges/trigger_mode" in full_info:
            ...
            self.base.w["trigger_mode"].set_options(
                [trig_modes[v] for v in index_values],
                index_values=index_values,
                value=index_values[0],
            )
            index_values = list(trig_out_modes)
            self.base.w["trigger_out_mode"].set_options(
                [trig_out_modes[v] for v in index_values],
                index_values=index_values,
                value=index_values[0],
            )
    ...
    def collect(self, parameters):
        if not self.disabled:
            parameters["trigger_mode"] = (
                self.base.v["trigger_mode"],
                self.base.v["trigger_out_mode"],
            )
        return super().collect(parameters)

    def display(self, parameters):
        if "trigger_mode" in parameters and not self.disabled:
            self.base.i["trigger_mode"], self.base.i["trigger_out_mode"] = parameters[
                "trigger_mode"
            ]
    ...
    def setup(self, parameters, full_info):
        ...
        if "parameter_ranges/clear_mode" in full_info:
            self.base.w[self.gui_name].set_options(
                dictionary.as_dict(full_info["parameter_ranges/clear_mode"]), index=0
            )
        else:
            self.disable()
...
class Settings_GUI(GenericCameraSettings_GUI):
    ...
    def get_basic_parameters(self, name):
        if name == "trigger_mode":
            return TriggerModeParameter(self)
        if name == "exposure":
...
class PvcamCameraDescriptor(ICameraDescriptor):
    ...
    def iterate_cameras(cls, verbose=False):
        if verbose:
            print("Searching for Pvcam cameras")
        try:
        ...
        if not cam_num:
            if verbose:
                print("Found no Pvcam cameras\n")
            return
        if verbose:
            print("Found {} Pvcam camera{}".format(cam_num, "s" if cam_num > 1 else ""))
        for name in cams:
            try:
