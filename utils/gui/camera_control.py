...
class GenericCameraCtl(container.QContainer):
    ...
    def __init__(
        self,
        cam_thread="camera",
        frame_src_thread=None,
        save_thread=None,
        snap_save_thread=None,
        resource_manager_thread=None,
        event_hooks_thread=None,
        frame_tag="frames/new",
        cam_name=None,
        settings=None,
    ):
        super().__init__()
    ...
    def setup(self):
        ...
        self.snap_saver = (
            ...
            if self.snap_save_thread
            else None
        )
        self.resource_manager = (
            controller.sync_controller(self.resource_manager_thread)
            if self.resource_manager_thread
            else None
        )
        self.ctl.subscribe_sync(
            self.receive_frame,
            self.frame_src_thread,
            tags=self.frame_tag,
            limit_queue=3,
        )
        self.ctl.subscribe_sync(
            lambda *args: self.recv_status_update(args[-1]),
            self.cam_thread,
            tags="status/connection",
        )
        self.ctl.subscribe_sync(
            lambda src, tag, val: self.plot_control(*val),
            tags="image_plotter/control",
            limit_queue=-1,
        )
        if self.resource_manager is not None:
            self.resource_manager.cs.add_resource(
                "frame/display",
                "standard",
                caption="Standard",
                src=self.frame_src_thread,
                tag=self.frame_tag,
                frame=None,
            )
            self.ctl.subscribe_sync(
                lambda *args: self.frames_sources_updates.emit(),
                srcs=self.resource_manager_thread,
                tags=["resource/added", "resource/removed"],
            )
            self.setup_activity_indicators()
        self.ctl.add_thread_method("toggle_saving", self.toggle_saving)
        self.ctl.add_thread_method(
            "get_saving_parameters",
            lambda mode="full": self.c["savebox"].collect_parameters(mode),
        )
    ...
    def setup_activity_indicators(self):
        ...
        self.resource_manager.cs.add_multicast_updater(
            ...
            "camera/connection",
            connection_updater,
            srcs=self.cam_thread,
            tags="status/connection",
        )
        self.resource_manager.cs.add_resource(
            "process_activity",
            "camera/acquisition",
            caption="Camera acquisition",
            short_cap="Acq",
            order=1,
        )
        acquisition_updater = status_updater(
            {"acquiring": "on", "setup": "pause", "cleanup": "pause"}
        )
        self.resource_manager.cs.add_multicast_updater(
            "process_activity",
            "camera/acquisition",
        ...
        if self.save_thread:
            ...
            saving_updater = status_updater(
                {"in_progress": "on", "stopping": "pause", "starting": "pause"}
            )
            self.resource_manager.cs.add_multicast_updater(
                "process_activity",
                "saving/streaming",
                saving_updater,
                srcs=self.save_thread,
                tags="status/saving",
            )
    ...
    def _call_event_hook(self, evt, *args, **kwargs):
        if self.event_hooks_thread is None:
            return None
        return controller.sync_controller(self.event_hooks_thread).cs.call_hook(
            evt, *args, **kwargs
        )
    ...
    def toggle_saving(
        ...
        if (self.saver and mode == "full") or (self.snap_saver and mode == "snap"):
            ...
            if mode == "full":
                if start:
                    if "settings" in self.c:
                        perform_status_check = (
                            self.c["settings"]
                            .collect_parameters()
                            .get("perform_status_check", False)
                        )
                    if self.event_hooks_thread is None or all(
                        self._call_event_hook("gui/saving/full/start", params=params)
                    ):
                        self.saver.csi.save_start(
                            params["path"],
                            path_kind=params["path_kind"],
                            batch_size=params["batch_size"],
                            append=params["append"],
                            format=params["format"],
                            format_parameters=params.get("format_parameters"),
                            filesplit=params["filesplit"],
                            save_settings=params["save_settings"],
                            perform_status_check=perform_status_check,
                        )
                else:
                ...
                if start:
                    if self.event_hooks_thread is None or all(
                        self._call_event_hook("gui/saving/snap/start", params=params)
                    ):
                        self.snap_saver.csi.save_start(
                            params["path"],
                            path_kind=params["path_kind"],
                            batch_size=1,
                            append=False,
    ...
    def send_snap_frame(self, source=None):
        ...
        if frame is not None:
            self.ctl.send_multicast(
                self.snap_save_thread,
                tag="frames/new/snap",
                value=FramesMessage([frame], chandim=frame.ndim - 2),
            )
    ...
    def get_thread_parameters(self):
        ...
        if self.saver:
            ...
            params["frames/status_line_check"] = self.saver.get_variable(
                "status_line_check", "none"
            )
        return params
    ...
    def receive_frame(self, src, tag, msg):
        ...
        if self.c["plotter_area"].update_expected():
            ...
            if self.c["plotter_area"].update_image():
                ...
                if self.resource_manager:
                    self.resource_manager.csi.update_resource(
                        "frame/display", "standard", frame=frame
                    )
