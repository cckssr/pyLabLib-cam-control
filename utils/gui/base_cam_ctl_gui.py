...
class ICameraSettings_GUI(container.QWidgetContainer):
    ...
    def setup(self, ctl):
        ...
        self.common_params = settings_tabs.add_tab("common", "Common").add_child(
            "common_params", param_table.ParamTable(self)
        )
        self.common_params.setup(add_indicator=True)
        self.advanced_params = settings_tabs.add_tab("advanced", "Advanced").add_child(
            "advanced_params", param_table.ParamTable(self)
        )
    ...
    def update_parameter_value(self, allow_diff_update=False):
        if self.v["auto_apply"]:
            self.cam_ctl.send_parameters(
                only_diff=allow_diff_update, dependencies=self._param_dependencies
            )
...
class GenericCameraStatus_GUI(param_table.StatusTable):
    ...
    def setup(self, ctl):
        ...
        self.add_status_line(
            "connection",
            label="Connection:",
            srcs=self.cam_ctl.cam_thread,
            tags="status/connection_text",
        )
    ...
    def show_parameters(self, params):
        ...
        if "frames/buffer_filled" in params and "buffer_size" in params:
            self.v["frames/buffstat"] = "{:d} / {:d}".format(
                params["frames/buffer_filled"], params["buffer_size"]
            )
        else:
