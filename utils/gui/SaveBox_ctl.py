...
class MessageLogWindow(container.QWidgetContainer):
    def setup(self, cam_ctl):
        ...
        self.params.add_text_edit(
            "recorded_events", value="", multiline=True, location=(1, 1, 1, 1)
        )
...
class FormatParametersWindow(container.QWidgetContainer):
    def setup(self):
        ...
        self.setWindowFlag(QtCore.Qt.WindowMaximizeButtonHint, False)
        self.setWindowFlag(QtCore.Qt.WindowMinimizeButtonHint, False)
        self.raw_params = self.add_group_box("raw_params", "Raw binary").add_child(
            "params", param_table.ParamTable(self), gui_values_path="raw"
        )
        ...
        self.raw_params.vs["override_dtype"].connect(
            lambda v: self.raw_params.set_enabled("dtype", v)
        )
...
_error_description = {
    ...
    "tiff_size_exceeded": (
        "TIFF exceeded 2GB",
        "TIFF files do not support sizes above 2GB. Consider using file splitting or switch to a different format.",
    ),
    "single_shot_overflow": (
        "Buffer overflow",
        "Single-shot buffer overflow. Consider expanding buffer size in Preferences.",
    ),
}
...
class SaveBox_GUI(container.QGroupBoxContainer):
    ...
    def setup(self, ctl):
        ...
        def browse_path():
            path, _ = QtWidgets.QFileDialog.getSaveFileName(
                self,
                "Save camera data...",
                **{qtkwargs.file_dialog_dir: self.v["path"]},
            )
            if path:
                self.v["path"] = path
        ...
        with self.params.using_new_sublayout(
            ...
            self.params.add_combo_box(
                ...
                options=["Overwrite", "Append", "Rename"],
                index_values=["overwrite", "append", "rename"],
                value="rename",
            )
        ...
        self.params.add_combo_box(
            "format",
            label="Format",
            options=["Raw binary", "TIFF", "Big TIFF"],
            index_values=["raw", "tiff", "bigtiff"],
        )
        ...
        def show_format_parameters():
            self.format_parameters_window.move(
                gui_utils.get_top_parent(self).rect().center()
                - self.format_parameters_window.rect().center()
            )
            self.format_parameters_window.show()

        self.params.add_child(
            "format_parameters",
            self.format_parameters_window,
            gui_values_path="format_parameters",
            location="skip",
        )
        ...
        self.params.add_combo_box(
            "stream_mode",
            label="Disk streaming",
            options={"cont": "Continuous", "single_shot": "Single-shot"},
            location=("next", 0, 1, 3),
        )
        ...
        def show_message_log():
            self.message_log_window.move(
                gui_utils.get_top_parent(self).rect().center()
                - self.message_log_window.rect().center()
            )
        ...
        self.params.add_child(
            "message_log_window",
            self.message_log_window,
            gui_values_path="message_log_window",
            location="skip",
        )
        ...
        def browse_snap_path():
            path, _ = QtWidgets.QFileDialog.getSaveFileName(
                self,
                "Save snapshot...",
                **{qtkwargs.file_dialog_dir: self.v["snap_path"]},
            )
            if path:
        ...
        def update_snap_path():
            ...
            self.params.set_enabled(
                ["snap_path", "snap_browse", "snap_make_folder", "snap_add_datetime"],
                not default_path,
            )
        ...
        with self.params.using_new_sublayout(
            ...
            self.params.vs["snap_displayed"].connect(
                lambda v: self.cam_ctl.toggle_saving(
                    mode="snap", source=self.v["snap_display_source"]
                )
            )
            ...
            self.params.add_combo_box(
                "snap_format",
                options=["Raw binary", "TIFF"],
                index_values=["raw", "tiff"],
                value="tiff",
            )
    ...
    def collect_parameters(self, mode="full", resolve_path=True):
        ...
        if resolve_path:
            ...
            if mode == "snap" or self.v["on_name_conflict"] == "rename":
                idx = None
                iname = self._expand_name(
                    name, idx, add_datetime=add_datetime, as_folder=make_folder
                )
                split = self.v["do_filesplit"]
                while self._is_name_taken(
                    os.path.join(folder, iname), ext, split=split, as_folder=make_folder
                ):
            ...
            if not file_utils.is_path_valid(params["path"]):
                if not self.cam_ctl.no_popup:
                    QtWidgets.QMessageBox.warning(
                        self,
                        "Invalid path",
                        "Invalid path: {}".format(params["path"]),
    ...
    def show_parameters(self, params):
        ...
        if just_stopped:  # record just stopped
            if not self.cam_ctl.no_popup and params.get(
                ...
                if params.get("frames/missed", 0) > 0 or params.get(
                    "frames/status_line_check", "na"
                ) not in {"na", "off", "none", "ok"}:
                    QtWidgets.QMessageBox.warning(
                        self,
                        "Problems with frames",
                        "Some frames are missing, duplicated, or out of order",
                        QtWidgets.QMessageBox.Ok,
                    )
    ...
    def update_display_source_options(self, reset_value=False):
        self.params.w["snap_display_source"].set_options(
            options=self.cam_ctl.get_frame_sources(), index=0 if reset_value else None
        )
    ...
    def set_all_values(self, values):
        ...
        if "saving" in values:
            del values["saving"]
        if (
            "default_snap_path" in values
        ):  # make sure it is changed first to not affect other settings
...
class SaveStatus_GUI(param_table.StatusTable):
    ...
    def setup(self, ctl):
        ...
        if self.cam_ctl.save_thread:
            ...
            self.add_status_line(
                "saving",
                label="Saving status:",
                srcs=self.cam_ctl.save_thread,
                tags="status/saving_text",
            )
            self.update_status_line("saving")

            def error_fmt(src, tag, val):
                self.w["issues"].setStyleSheet(
                    "color: red; font-weight: bold" if val[0] != "none" else ""
                )
    ...
    def show_parameters(self, params):
        ...
        if missed_frames is not None:
            self.w["frames/missed"].setStyleSheet(
                "color: red; font-weight: bold" if missed_frames else ""
            )
        if "frames/queue_ram" in params:
            self.v["frames/ram_status"] = "{:.0f} / {:.0f} Mb".format(
        ...
        if "frames/status_line_check" in params:
            ...
            slc_statuses = {
                ...
                "ok": "OK",
                "na": "No frames",
                "none": "No status line found",
                "out_of_oder": "Frames out of order",
                "still": "Duplicate frames",
                "skip": "Skipping frames",
            }
