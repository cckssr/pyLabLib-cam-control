...
class SettingsEditor(widgets.QWidgetContainer):
    ...
    def setup(self, main_frame):
        ...
        for cam in self.settings.get("cameras", {}):
            ...
            self.cam_tabs[name] = self.tabs.add_tab(
                "cam/" + cam, name, widget=widgets.ParamTable(self)
            )
    ...
    def add_bool_parameter(
        ...
        self.decorate_parameter(
            table, name, default=default, descfunc=lambda v: description[1 if v else 0]
        )
    ...
    def add_choice_parameter(
        ...
        self.decorate_parameter(
            ...
            name,
            default=default,
            descfunc=lambda v: description.get(v, v) if v != -1 else "N/A",
        )

    def add_integer_parameter(
        self, table: widgets.ParamTable, name, label, limits=(0, None), default=0
    ):
    ...
    def setup_settings(self, table: widgets.ParamTable):
        ...
        self.add_choice_parameter(
            ...
            "Color theme",
            {"standard": "Standard", "dark": "Dark", "light": "Light"},
            default="dark",
        )
        self.add_bool_parameter(
            table, "interface/expandable_edits", "Expandable text boxes", default=True
        )
        table.add_spacer(10)
        self.add_choice_parameter(
            table,
            "interface/datetime_path/file",
            "Add date/time file method",
            {"pfx": "Prefix", "sfx": "Suffix", "folder": "Folder"},
            description={
                "pfx": "Add as a prefix",
                "sfx": "Add as a suffix",
                "folder": "Create separate folder",
            },
            default="sfx",
        )
        self.add_choice_parameter(
        ...
        self.add_choice_parameter(
            ...
            description={
                "keep": "Keep unchanged",
                "cut": "Cut row off",
                "zero": "Set row to zero",
                "median": "Set row to image median",
                "duplicate": "Duplicate previous row",
            },
        )
        self.add_choice_parameter(
            table,
            ...
            "ROI entry method",
            {"minmax": "Min-Max", "minsize": "Min-Size", "centersize": "Center-Size"},
            description={
                "minmax": "Minimal and maximal coordinates",
                "minsize": "Minimal coordinates and size",
                "centersize": "Center coordinates and size",
            },
            default="minmax",
        )
        if cam_table:
            ...
            self.add_float_parameter(
                table,
                "camera/params/misc/buffer/min_size/time",
                "Frame buffer (s)",
                limits=(0.2, None),
                default=1.0,
            )
            self.add_float_parameter(
                table,
                "camera/params/misc/loop/min_poll_period",
                "Poll period (s)",
                limits=(0.01, 1),
                default=0.05,
                fmt=".2f",
