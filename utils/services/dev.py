...
def take_screenshots(src):
    ...
    if src.compact_interface:
        ...
        src.ctl.sleep(0.1)
        gui_utils.get_screenshot(widget=src.c["plot_tabs"]).save(
            sfolder + "interface_image_display.png"
        )
        gui_utils.get_screenshot(widget=src.c["params_loading_settings"]).save(
            sfolder + "interface_footer.png"
        )
        src.c["control_tabs"].set_by_name("cam_tab")
        src.ctl.sleep(0.1)
        gui_utils.get_screenshot(
            widget=src.c["cam_controller/settings"].parentWidget(),
            border=(17, 17, 38, 3),
        ).save(sfolder + "interface_camera_settings.png")
        ...
        src.ctl.sleep(0.1)
        gui_utils.get_screenshot(widget=src.c["control_tabs/proc_tab"]).save(
            sfolder + "interface_processing.png"
        )
        src.c["control_tabs"].set_by_name("proc_tab")
        src.ctl.sleep(0.1)
        crop(
        ...
        src.ctl.sleep(0.1)
        crop(
            gui_utils.get_screenshot(widget=src.c["control_tabs/proc_tab"]),
            top=512,
            bottom=810,
        ).save(sfolder + "interface_time_plot.png")
        src.v["plotting/enable"] = False
        src.ctl.sleep(0.1)
        crop(
            gui_utils.get_screenshot(widget=src.c["activity_indicator"], border=3),
            bottom=195,
        ).save(sfolder + "interface_activity.png")
        src.c["control_tabs"].set_by_name("filter.filt/ctl_tab")
        src.ctl.sleep(0.1)
        crop(
            gui_utils.get_screenshot(
                widget=src.c["control_tabs/filter.filt/ctl_tab"], border=(9, 9, 29, 3)
            ),
            bottom=332,
        ).save(sfolder + "interface_filter.png")
        src.c["control_tabs"].set_by_name("plugins")
        src.ctl.sleep(0.1)
        gui_utils.get_screenshot(
            widget=src.c["control_tabs/plugins/trigger_save.trigsave/params"],
            border=(9, 9, 29, 3),
        ).save(sfolder + "interface_save_trigger.png")
        src.call_extra("settings_editor")
        src.settings_editor.tabs.setCurrentIndex(1)
        src.ctl.sleep(0.1)
        gui_utils.get_screenshot(window=src.settings_editor).save(
            sfolder + "interface_preferences.png"
        )
        ...
        if "show_attributes_window" in src.c["cam_controller/settings"].advanced_params:
            ...
            src.ctl.sleep(0.1)
            gui_utils.get_screenshot(window=window).save(
                sfolder + "interface_camera_attributes.png"
            )
            window.tabs.set_by_name("value_props")
            src.ctl.sleep(0.1)
            gui_utils.get_screenshot(window=window).save(
                sfolder + "interface_camera_attributes_settings.png"
            )
            src.tutorial_box.close()
