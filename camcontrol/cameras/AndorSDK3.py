...
class Status_GUI(GenericCameraStatus_GUI):
    ...
    def show_parameters(self, params):
        ...
        if "missed_frames" in params:
            ...
            self.w["buffer_overflows"].setStyleSheet(
                "font-weight: bold" if params["missed_frames"].overflows else ""
            )
...
class AndorSDK3CameraDescriptor(ICameraDescriptor):
    ...
    def iterate_cameras(cls, verbose=False):
        if verbose:
            print("Searching for Andor SDK3 cameras")
        try:
            cam_num = Andor.get_cameras_number_SDK3()
        except (Andor.AndorError, OSError):
            if verbose:
                print(
                    "Error loading or running the Andor SDK3 library: required software (Andor Solis) must be missing\n"
                )
            if verbose == "full":
                cls.print_error()
            return
