...
class Settings_GUI(GenericCameraSettings_GUI):
    ...
    def setup_settings_tables(self):
        ...
        self.add_parameter(
            cam_gui_parameters.EnumGUIParameter(
                self,
                "readout_speed",
                "Readout speed",
    ...
    def setup_settings_tables(self):
        ...
        self.add_parameter(
            cam_gui_parameters.BoolGUIParameter(
                self, "defect_correct_mode", "Defect correction", default=True
            ),
            "advanced",
            row=-1,
        )
...
class DCAMCameraDescriptor(ICameraDescriptor):
    ...
    def iterate_cameras(cls, verbose=False):
        if verbose:
            print("Searching for DCAM cameras")
        try:
        ...
        if cam_num == 0:
            if verbose:
                print("Found no DCAM cameras\n")
            return
        if verbose:
            print("Found {} DCAM camera{}".format(cam_num, "s" if cam_num > 1 else ""))
