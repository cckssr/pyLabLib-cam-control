...
class MightexSSeriesCameraDescriptor(ICameraDescriptor):
    ...
    def iterate_cameras(cls, verbose=False):
        if verbose:
            print("Searching for Mightex cameras")
        try:
            cams = Mightex.list_cameras_s()
        except (Mightex.MightexError, OSError, AttributeError):
            if verbose:
                print(
                    "Error loading or running the Mightex S Series library: required software must be missing\n"
                )
            if verbose == "full":
                cls.print_error()
            return
        if len(cams) == 0:
            if verbose:
                print("Found no Mightex cameras\n")
            return
        cam_num = len(cams)
        if verbose:
            print("Found {} Mightex camera{}".format(cam_num, "s" if cam_num > 1 else ""))
        for i, cdesc in enumerate(cams):
            if verbose:
                print(
                    f"Checking Mightex camera idx={i}\nModel {cdesc.model},   serial {cdesc.serial}"
                )
            yield None, cdesc
    ...
    def generate_description(cls, idx, cam=None, info=None):
        ...
        cam_desc["display_name"] = f"{info.model} {info.serial}"
        cam_name = f"mightex_sseries_{idx}"
