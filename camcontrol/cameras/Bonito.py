...
class BonitoCameraSettings_GUI(GenericCameraSettings_GUI):
    ...
    def get_basic_parameters(self, name):
        ...
        if name == "roi":
            return cam_gui_parameters.ROIGUIParameter(
...
class BonitoIMAQCameraDescriptor(ICameraDescriptor):
    ...
    def iterate_cameras(cls, verbose=False):
        if verbose:
            print("Searching for IMAQ Bonito cameras")
        try:
        ...
        if not cam_num:
            if verbose:
                print("Found no IMAQ cameras\n")
            return
        if verbose:
            print("Found {} IMAQ camera{}".format(cam_num, "s" if cam_num > 1 else ""))
