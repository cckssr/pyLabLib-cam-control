    ...
    def _add_attribute(self, name, attribute, value):
        ...
        if attribute.kind in {"Integer", "Large Integer"}:
            ...
            self.add_bool_parameter(
                name, attribute.name, default=attribute.default, indicator=indicator
            )
...
class PicamCameraDescriptor(ICameraDescriptor):
    ...
    def iterate_cameras(cls, verbose=False):
        if verbose:
            print("Searching for Picam cameras")
        try:
        ...
        if len(cams) == 0:
            if verbose:
                print("Found no Picam cameras\n")
            return
        cam_num = len(cams)
        if verbose:
            print("Found {} Picam camera{}".format(cam_num, "s" if cam_num > 1 else ""))
