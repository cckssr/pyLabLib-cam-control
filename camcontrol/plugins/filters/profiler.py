...
class BeamProfileFilter(base.ISingleFrameFilter):
    ...
    def setup(self):
        ...
        self.add_parameter(
            "width", label="Averaging width", kind="int", limit=(1, None), default=10
        )
        self.add_parameter(
            "show_map_info",
            label="Showing",
            kind="select",
            options={"frame": "Frame", "data": "Data profile", "fit": "Fit profile"},
        )
    ...
    def set_parameter(
        self, name, value
    ):  # called automatically any time a GUI parameter or an image cross position are changed
        """Set filter parameter with the given name"""
        super().set_parameter(
            name, value
        )  # default parameter set (store the value in ``self.p`` dictionary)
        if (
            name in ["linepos", "track_lines", "show_map_info"]
    ...
    def process_frame(
        self, frame
    ):  # called automatically whenver a new frame is received from the camera
        ...
        if self.p["show_map_info"] == "frame":  # showing the original frames
            ...
            self.change_rectangle(
                "x_selection",
                center=((rs[1] + rs[0]) / 2, nc / 2),
                size=(rs[1] - rs[0], nc),
                visible=True,
            )
            self.change_rectangle(
                "y_selection",
                center=(nr / 2, (cs[1] + cs[0]) / 2),
                size=(nr, cs[1] - cs[0]),
                visible=True,
            )
            self.select_plotter("frame")
