...
class FFTBandpassFilter(base.ISingleFrameFilter):
    ...
    _class_description = (
        "High- and low-pass filter operating in Fourier domain. Filters out variations below minimal width and above maximal width. "
    ...
    def setup(self):
        ...
        self.add_parameter(
            ...
            label="Filter kind",
            kind="select",
            options={"smooth": "Smooth (DoG)", "hard": "Hard"},
        )
        self.add_parameter(
            "show_info",
            label="Showing",
    ...
    def set_parameter(self, name, value):
        ...
        if self.p["minwidth"] > self.p["maxwidth"]:
            self.p["minwidth"], self.p["maxwidth"] = (
                self.p["maxwidth"],
                self.p["minwidth"],
            )
        self._update_mask()
...
)  # buffer is guranteed to stay constant during execution, so can lift GIL; parallel mode is unstable, shouldn't be used
def _movavg(buffer):
    n, r, c = buffer.shape
    l = n // _movavg_per
    ...
    def process_buffer(self, buffer, start, filled):
        ...
        if buffer.ndim > 3:
            return np.concatenate(
                [
                    self.process_buffer(buffer[..., ch], start, filled)[..., None]
...
class MovingAccumulatorFilter(base.IMultiFrameFilter):
    ...
    def process_buffer(self, buffer):
        ...
        if self.p["kind"] == "std" and len(buffer) > 1:
...
class FastMovingAverageSubtractionFilter(base.IRingMultiFrameFilter):
    ...
    _class_description = (
        "Averages two consecutive frame blocks into two individual frames and takes their difference. "
...
class TimeMapFilter(base.IMultiFrameFilter):
    ...
    _class_description = (
        "Plots a time dependence of a line cut as a 2D map. "
    ...
    def setup(self):
        ...
        self.add_parameter(
            ...
            label="Orientation",
            kind="select",
            options={"rows": "Rows", "cols": "Columns"},
        )
        ...
        self.add_parameter(
            ...
            label="Showing",
            kind="select",
            options={"map": "Time map", "frame": "Frame"},
        )
    ...
    def set_parameter(self, name, value):
        ...
        self.reshape_buffer(buffer_size, buffer_step)
        if (
            name in ["linepos", "orientation", "track_lines"]
            and self.p["show_map_info"] == "frame"
            and self.p["track_lines"]
            and self.p["linepos"]
        ):
    ...
    def process_buffer(self, buffer):
        ...
        if self.p["show_map_info"] == "frame":
            ...
            self.change_rectangle(
                ...
                size=np.abs(corners[1] - corners[0]),
                visible=True,
            )
            self.select_plotter("frame")
            return frame
