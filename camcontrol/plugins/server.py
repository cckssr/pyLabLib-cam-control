...
class ServerCommThread(controller.QTaskThread):
    ...
    def _build_error_message(self, error):
        reply = dictionary.Dictionary(
            {
                "purpose": "error",
                "parameters": {"name": error.kind, "args": error.err_args},
            }
        )
        if error.desc:
    ...
    def recv_message(self):
        ...
        if not isinstance(msg, dict):
            raise IncomingMessageError(
                "wrong_format", "Wrong incoming message format", {"value": msg}
            )
        ...
        if "payload" in msg:
            ...
            try:
                ...
                raise IncomingMessageError(
                    "wrong_type",
                    "Wrong payload specification",
                    {"value": msg["payload"]},
                )
            except net.SocketTimeout:
                raise IncomingMessageError(
                    "wrong_argument",
                    "Payload size is smaller than specified",
                    {"value": nbytes},
                )
    ...
    def process_save_request(self, name, args):
        ...
        if name == "start":
            try:
                ...
                raise IncomingMessageError(
                    "wrong_type",
                    "Wrong type for the supplied acquisition parameters",
                    {"value": args},
                )
            return "success"
        if name == "stop":
    ...
    def process_cam_request(self, name, args):
        ...
        if name == "param/set":
            args = dictionary.Dictionary(args).map_self(
    ...
    def process_stream_request(self, name, args):
        ...
        if name == "buffer/setup":
            if "size" in args:
                self.frames_accum_size = self.get_message_key(
                    args, "size", branch="parameters/args", dtype="int"
                )
            ...
            if n:
                ...
                return {
                    "filled": n,
                    "size": self.frames_accum_size,
                    "first_index": int(fidx[0]),
                    "last_index": int(lidx[0]),
                }
            else:
                return {
                    "filled": 0,
...
class ServerPlugin(base.IPlugin):
        ...
        while self.port < max_port:
            try:
                ...
                net.listen(
                    ...
                    self.port,
                    self.connect,
                    wait_callback=self.ctl.check_messages,
                )
                break
            except OSError:
                self.port += 1
    ...
    def connect(self, socket):
        ...
        ServerCommThread(
            self.thread_name_gen(self.ctl.name + ".comm_thread"),
            kwargs={"socket": socket, "plugin": self},
        ).start()
