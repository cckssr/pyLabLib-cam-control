...
class FilterPanel(widgets.QFrameContainer):
    ...
    def setup(self, plugin, filters, plotter=None):
        ...
        self.params.add_combo_box(
            "filter_id", label="Filter:", options=filters, out_of_range="ignore"
        )
        ...
        self.params.add_toggle_button(
            "enabled", "Enable", value=True, location=("next", 0, 1, "end")
        )
        ...
        enable_filter(True)
        self.filter_params_table = self.add_child(
            "filter_params",
            widgets.ParamTable(self),
            gui_values_path="current_filter_params",
        )
        ...
        self.add_spacer(20)
        self.filter_status_table = self.add_child(
            "filter_status",
            widgets.ParamTable(self),
            gui_values_path="current_filter_status",
        )
    ...
    def setup_filter(self, name, description):
        ...
        for pdesc in description.get("gui/parameters", []):
            ...
            if pkind == "text":
                if pind:
                    self.filter_status_table.add_text_label(
                        pname, value=pdefault or "", label=plabel
                    )
                else:
                    self.filter_params_table.add_text_edit(
                        pname, value=pdefault or "", label=plabel
                    )
                ...
                if pkind == "int" and pfmt is None:
                    pfmt = "int"
                if pind:
                    self.filter_status_table.add_num_label(
                        pname, value=pdefault or 0, formatter=pfmt, label=plabel
                    )
                else:
                    self.filter_params_table.add_num_edit(
                        pname,
                        value=pdefault or 0,
                        limiter=(plimit[0], plimit[1], "coerce", pkind),
                        formatter=pfmt,
                        label=plabel,
                    )
            elif pkind == "button":
    ...
    def update_indicators(self, values, data=None):
        ...
        if self.plotter is not None and data is not None:
            ...
            for n, p in new_rects.items():
                self.plotter.plt.set_rectangle(
                    n,
    ...
    def _store_plotter_selector(self, plotter):
        ...
        if selector is not None:
            self.set_aux(
                plotter,
                ("plotter", selector),
                self.filter_defaults.get((plotter, "__plotter__"), {}),
            )
    ...
    def set_all_values(self, values):
        ...
        if "defaults" in values:
            self.load_default_values(
                self.current_filter[0] if self.current_filter else None,
                exclude=["__plotter__"],
            )
...
class FilterThread(controller.QTaskThread):
    ...
    def setup_task(self, src, tag="frames/new", tag_out=None, settings=None):
        ...
        self.subscribe_commsync(
            self.receive_message, srcs=src, tags=tag, limit_queue=20, priority=-5
        )
    ...
    def receive_message(self, src, tag, msg):
        ...
        if self.enabled and self.fctl is not None:
            ...
            if self.single_frame:
                self._receive_frames(
                    msg.last_frame(),
                    msg.metainfo.get("status_line"),
                    single=True,
                    chandim=msg.mi.chandim,
                )
            else:
                self._receive_frames(
                    msg.frames,
        ...
        self._last_frame = remove_status_line(
            msg.last_frame(),
            msg.metainfo.get("status_line"),
            policy=self.status_line_policy,
            copy=True,
        )
    ...
    def get_new_data(self, only_new=True):
        ...
        if "frame" in data:
            self.send_multicast(
                dst="any",
                tag=self.tag_out,
                value=self.frames_src.build_message(
                    data["frame"], self._last_frame_index, source=self.name
                ),
            )
    ...
    def set_parameter(self, name, value):
        ...
        if self.fctl is not None and name in self.get_variable(
            "filter_props/parameters", default=[]
        ):
...
class FilterPlugin(base.IPlugin):
    ...
    def setup(self):
        ...
        self.filter_thread = FilterThread(
            self.ctl.name + ".filter_thread",
            kwargs={"src": self.extctls["slowdown"].name, "settings": self.parameters},
        )
        ...
        self.setup_gui_sync()
        self.extctls["resource_manager"].cs.add_resource(
            "frame/display",
            self.full_name,
            ctl=self.ctl,
            caption=self.caption,
            src=self.filter_thread.name,
            tag=self.filter_thread.tag_out,
            frame=None,
        )
        self.extctls["resource_manager"].cs.add_resource(
            "process_activity",
            "processing/" + self.full_name,
            ctl=self.ctl,
            caption=self.caption,
            order=10,
        )
    ...
    def setup_gui(self):
        ...
        self.proc_indicator = self.plot_tab.add_child(
            "processing_indicator",
            ProcessingIndicator_ctl.ProcessingIndicator_GUI(self.plot_tab),
        ...
        with self.plotter.using_layout("sidebar"):
            ...
            self.display_settings_table.setup(
                slowdown_thread=self.extctls["slowdown"].name, period_update_tag=None
            )
            self.display_settings_table.params.vs["display_update_period"].connect(
    ...
    def set_all_values(self, values):
        if (
            values.get("ctl_tab/enabled", False)
            and values.get("ctl_tab/loaded", False)
            and not self._gui_started
        ):
    ...
    def update_plots(self, force=False):
        ...
        if self._update_image(values=values, data=data):
            self.extctls["resource_manager"].csi.update_resource(
                "frame/display", self.full_name, frame=data["frame"]
            )
