...
class TriggerSavePlugin(base.IPlugin):
    ...
    def setup(self):
        ...
        self.setup_gui_sync()
        self.ctl.subscribe_commsync(
            lambda *args: self._update_frame_sources(),
            srcs=self.extctls["resource_manager"].name,
    ...
    def setup_gui(self):
        ...
        if location == "plugin_tab":
            ...
            self.table = self.gui.add_plugin_box(
                "params", caption, cache_values=True, parent=savebox, index="next"
            )
        ...
        self.table.add_combo_box(
            "trigger_mode",
            options={m: trig_mode_names[m] for m in self.trig_modes},
            label="Trigger mode",
        )
        self.table.add_num_edit(
            "period",
            10,
            limiter=(0.1, None, "coerce"),
        ...
        self.table.add_num_edit(
            ...
            0,
            formatter=("float", "auto", 4),
            label="Trigger threshold",
        )
        self.table.add_num_edit(
            "dead_time",
            10,
            limiter=(0, None, "coerce"),
            formatter=("float", "auto", 1),
            label="Dead time (s)",
        )
        ...
        def setup_gui_state():
            ...
            self.table.set_enabled(
                "enabled",
                not (trigger_mode == "image" and self.table.v["frame_source"] == -1),
            )
        ...
        if update_subscriptions:
            ...
            for n, v in sources.items():
                if n not in self._frame_sources:
                    sid = self.ctl.subscribe_commsync(
                        make_frame_recv_func(n),
                        srcs=v["src"],
                        tags=v["tag"],
                        limit_queue=1,
                    )
    ...
    def check_timer_trigger(self):
        ...
        if enabled and self.table.v["trigger_mode"] == "timer":
            ...
            if not (saving or self._last_video) and (
                self._last_save_timer is None or t > self._last_save_timer + period
            ):
    ...
    def _update_trigger_status(self, status):
        ...
        ):  # check (cached) value first to avoid unnecessary calls to GUI thread
            self.table.v["status"] = status
