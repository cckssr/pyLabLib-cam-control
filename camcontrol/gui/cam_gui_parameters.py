...
class IGUIParameter:
    ...
    def _update_value(self, v):
        self.settings.update_parameter_value(
            allow_diff_update=self._is_diff_update_allowed(self._current_value, v)
        )
    ...
    def add(self, base, row=None):
        ...
        if self.indicator:
            ...
            base.add_num_edit(
                ...
                value=self.default,
                label=self.label,
                limiter=self.limit + ("coerce", "int"),
                formatter="int",
                add_indicator=bool(self.add_indicator),
                location={"indicator": "next_line", "widget": row}
                if self.add_indicator == "next_line"
                else row,
            )
        super().add(base)
    ...
    def add(self, base, row=None):
        ...
        if self.indicator:
            ...
            base.add_combo_box(
                ...
                value=self.default,
                label=self.label,
                options=self.olabels,
                index_values=self.ovalues,
                add_indicator=bool(self.add_indicator),
                location={"indicator": "next_line", "widget": row}
                if self.add_indicator == "next_line"
                else row,
...
class ROIGUIParameter(IGUIParameter):
        ...
        with base.using_new_sublayout("roi_buttons", "hbox", location=("next", 0, 1, "end")):
            ...
            def _select_roi(v):
                if "plotter_area" in self.settings.cam_ctl.c:
                    self.settings.cam_ctl.c["plotter_area"].enable_selection_frame(
                        v, image_bound=False
                    )
                    self._switch_to_standard_tab(v)

            base.add_toggle_button(
                "select_roi",
                "Select in image",
                add_indicator=False,
                location=("next", 2, 1, 1),
            ).get_value_changed_signal().connect(_select_roi)
            base.add_padding()

            @controller.exsafe
            def _full_roi():
                xp, yp = self.roi_ctl.get_value()
                self.roi_ctl.set_value(
                    (
                        (self.roi_ctl.xlim[0], self.roi_ctl.xlim[1], xp[2]),
                        (self.roi_ctl.ylim[0], self.roi_ctl.ylim[1], yp[2]),
                    )
                )
    ...
    def on_changed(self):
        if self.detector_size is not None:
            ...
            current_roi_size = np.array(
                [
                    current_roi[0][1] - current_roi[0][0],
                    current_roi[1][1] - current_roi[1][0],
                ]
            )
            current_roi_center = (
                np.array(
            ...
            cam_ctl.plot_control(
                "rectangles/set", ("new_roi", current_roi_center, current_roi_size)
            )
            cam_ctl.plot_control(
                "rectangles/" + ("show" if self.settings.v["show_gui_roi"] else "hide"),
                ("new_roi",),
            )
    ...
    def _switch_to_standard_tab(self, show=True):
        if show:
            self.settings.cam_ctl.ctl.call_thread_method(
                "toggle_tab", "plot_tabs", "standard_frame"
            )
    ...
    def setup(self, parameters, full_info):
        if not all(p in parameters for p in ["roi", "roi_limits"]):
            ...
            self.roi_ctl.set_limits(
                xlim=(0, hlim.max),
                ylim=(0, vlim.max),
                minsize=(hlim.min, vlim.min),
                maxbin=maxbin,
            )
...
class AttributesBrowserGUIParameter(IGUIParameter):
    ...
    def add(self, base):
        ...
        self.base.add_button(
            "show_attributes_window", "Show attributes", add_indicator=False
        ).get_value_changed_signal().connect(lambda: self._show_window())
        self.base.add_child(
            "attributes_window",
            self.attr_window,
            location="skip",
            gui_values_path="attributes_window",
        )
