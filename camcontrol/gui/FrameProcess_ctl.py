...
class FrameProccess_GUI(container.QGroupBoxContainer):
    ...
    def setup(self, process_thread, settings=None):
        ...
        self.params.add_combo_box(
            "method",
            options=["Snapshot", "Running"],
            index_values=["snapshot", "running"],
            label="Method:",
        )
        ...
        with self.params.using_new_sublayout("background_saving_row", "hbox"):
            self.params.add_combo_box(
                "background_saving",
                options=["None", "Only background", "Background + source"],
                index_values=["none", "background", "all"],
                label="Snap save:",
            )
        ...
        self.params.vs["grab_background"].connect(
            lambda: self.image_processor.ca.grab_snapshot_background()
        )
        # Timer
