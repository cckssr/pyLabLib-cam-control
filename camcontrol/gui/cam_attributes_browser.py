...
class CamAttributesBrowser(widgets.QWidgetContainer):
    def setup(self, cam_ctl):
        ...
        self.values_scroll_area.verticalScrollBar().valueChanged.connect(
        ...
        self.props_tab = self.tabs.add_tab(
            "value_props", "Settings", widget=widgets.ParamTable(self)
        )
        self.props_tab.setup()
        self.props_scroll_area = self.props_tab.add_child(
            "scroll_area", widgets.QScrollAreaContainer(self)
        )
        self.props_scroll_area.setup()
        self.props_scroll_area.verticalScrollBar().valueChanged.connect(
    ...
    def _on_parameters_updated(self, *_):
        self._activity_state = (
            "updated" if self._activity_state == "activated" else self._activity_state
        )

    def _setup_connected_camera(self):
        if not self._cam_setup_done:
            self.ctl.subscribe_direct(
                self._on_parameters_updated,
                srcs=self.cam_ctl.dev.name,
                tags="changed/parameters",
            )
    ...
    def _add_dummy_checkbox(self, table, location=-1):
        w = QtWidgets.QCheckBox(
            "", parent=self.params_table
        )  # dummy checkbox as a workaround for aligning tabs layouts
    ...
    def decorate_parameter(self, name, label, indicator=False):
        ...
        if not indicator:
            self.params_table.add_button(
                "s/" + name, caption="Set", add_indicator=False, location=(-1, 2, 1, 1)
            )
            self.params_table.add_text_label("e/" + name, location=(-1, 4, 1, 1))
            self.params_table.w["e", name].setAlignment(QtCore.Qt.AlignCenter)
            self.params_table.w["e", name].clicked.connect(
                lambda: self._clear_error_indicator(name)
            )
            self.props_table.add_combo_box(
                "p_pause/" + name,
                options={"none": "None", "restart": "Restart", "clear": "Clear"},
                value="clear",
                location=(-1, 1, 1, 1),
            )
            self.props_table.add_combo_box(
    ...
    def _parloc(self, indicator):
        return (
            {"widget": ("next", 2, 1, 3), "label": ("next", 0)}
            if indicator
            else {"widget": ("next", 0, 1, 2), "indicator": ("next", 3)}
        )

    def add_bool_parameter(self, name, label, default=False, indicator=False):
        """Add a boolean attribute row"""
        if indicator:
            self.params_table.add_enum_label(
                "v/" + name,
                value=default,
                options={False: "Off", True: "On"},
                prep=bool,
                label=label,
                location=self._parloc(indicator),
            )
        else:
            self.params_table.add_check_box(
                "v/" + name,
                "",
                value=default,
                label=label,
    ...
    def add_choice_parameter(self, name, label, options, default=None, indicator=False):
        ...
        if indicator:
            ...
            self.params_table.add_combo_box(
                "v/" + name,
                value=default,
                label=label,
                options=olabels,
    ...
    def add_integer_parameter(self, name, label, limits=(0, None), default=0, indicator=False):
        ...
        if indicator:
            self.params_table.add_num_label(
                ...
                value=default,
                label=label,
                limiter=limits + ("coerce", "int"),
                formatter="int",
                location=self._parloc(indicator),
            )
        else:
            self.params_table.add_num_edit(
                "v/" + name,
                value=default,
                label=label,
                limiter=limits + ("coerce", "int"),
    ...
    def add_string_parameter(self, name, label, default="", indicator=False):
        ...
        if indicator:
            self.params_table.add_text_label(
                "v/" + name,
                value=default,
                label=label,
                location=self._parloc(indicator),
            ...
            self.params_table.add_text_edit(
                "v/" + name,
                value=default,
                label=label,
                location=self._parloc(indicator),
    ...
    def add_float_parameter(
        self, name, label, limits=(0, None), fmt=".3f", default=0, indicator=False
    ):
    ...
    def _set_camera_attributes(self, names):
        ...
        for n in names:
            ...
            if self.props_table.v["p_pause", n] == "restart":
                pause = "pause"
        self.cam_ctl.dev.ca.apply_parameters(
            {"camera_attributes/" + n: self.params_table.v["v", n] for n in names},
            pause=pause,
        )
