...
class PlotControl_GUI(container.QGroupBoxContainer):
    ...
    def setup_gui_state(self):
        ...
        self.params.set_enabled(
            ["source", "skip_count", "roi/enable", "update_plot", "reset_history"],
            enabled,
        )
        ...
        if enabled and roi_enabled:
            self.ctl.send_multicast(
                tag="image_plotter/control", value=("rectangles/show", "mean_plot_roi")
            )
        else:
            self.ctl.send_multicast(
                tag="image_plotter/control", value=("rectangles/hide", "mean_plot_roi")
            )

    def _update_roi_display(self, center, size):
        self.ctl.send_multicast(
            tag="image_plotter/control",
            value=("rectangles/set", ("mean_plot_roi", center, size)),
        )
        ...
        for i, (ch, l, en) in enumerate(zip(channels, labels, enabled)):
            if ch not in self.plot_lines:
                if ch == "idx":
                    ...
                    self.plot_markers[ch] = self.plot_window.plot(
                        [], [], symbolBrush=col, symbol="o", symbolSize=5, pxMode=True
                    )
