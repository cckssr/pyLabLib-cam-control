...
class FramePreproccessBinning_GUI(container.QGroupBoxContainer):
    ...
    def setup(self, preprocess_thread):
        ...
        self.params.add_combo_box(
            "spat_bin_mode",
            options=bin_mode_names,
            index_values=bin_modes,
            value="mean",
            label="Spatial binning mode:",
        )
        ...
        def setup_binning():
            ...
            self.image_preprocessor.ca.setup_binning(
                spat_bin,
                spat_bin_mode,
                time_bin,
                time_bin_mode,
                dtype="float" if convert_to_float else None,
            )
        ...
        self.ctl.res_mgr.cs.add_resource(
            "process_activity",
            "processing/binning",
            caption="Binning",
            short_cap="Bin",
...
class FramePreproccessSlowdown_GUI(container.QGroupBoxContainer):
    ...
    def recv_parameters(self):
        ...
        self.i["slowdown_buffer"] = "{} / {}".format(
            self.frame_slowdown.v["buffer/used"], self.frame_slowdown.v["buffer/filled"]
        )
